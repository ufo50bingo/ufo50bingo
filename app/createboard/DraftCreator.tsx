...
export default function DraftCreator({
    ...
    const [rawDifficultyCountsByPlayer, setDifficultyCountsByPlayer] = useState<ReadonlyArray<Map<Difficulty, number>>>([
        new Map([["easy", 3], ["medium", 3], ["hard", 3], ["veryhard", 3]]),
        new Map([["easy", 4], ["medium", 4], ["hard", 3], ["veryhard", 2]]),
        new Map([["easy", 0], ["medium", 0], ["hard", 0], ["veryhard", 0]]),
        new Map([["easy", 0], ["medium", 0], ["hard", 0], ["veryhard", 0]]),
        new Map([["easy", 0], ["medium", 0], ["hard", 0], ["veryhard", 0]]),
    ]);
    ...
    const difficultySum = difficultyCountsByPlayer.reduce(
        (acc, difficultyCount) => acc + difficultyCount
            .values()
            .reduce((playerAcc, next) => playerAcc + next, 0),
        numGenerals,
    );

    const playerToDifficultyToGameToGoal = useMemo<PlayerToDifficultyToGameToGoal>(() => {
        const finalValue: PlayerToDifficultyToGameToGoal = difficultyCountsByPlayer.map(_ => new Map());
        pasta.forEach((group) =>
            group.forEach((goal) => {
    ...
    const hasTooFewGoals = playerToAvailableGoalDifficultyCounts.some(
        (difficultyCounts, playerIndex) =>
            difficultyCounts.entries().some(
                ([difficulty, availableCount]) =>
                    availableCount < (difficultyCountsByPlayer[playerIndex].get(difficulty) ?? 0)
            )
    );
    ...
    useEffect(() => {
        onChangePasta(
            hasWrongSum || hasTooFewGoals
                ? null
                : createDraftPasta(pasta, numGenerals, playerToDifficultyToGameToGoal, difficultyCountsByPlayer)
        );
    }, [
        hasWrongSum,
        hasTooFewGoals,
    ...
    return (
        <Stack>
            <Alert title="Standard Draft">
                To construct a Standard Draft game, one player should share their view of this page, and then players
                should alternate drafting games up to the agreed upon limit. If desired, players can then protect/ban
                games.<br />
                <br />
                For each game, the first checkbox indicates whether the first player has drafted the game, and so on.<br />
                Each game can be drafted by only one player.<br />
                <br />
                If you want to play with more than 2 players, <u><strong>you must also manually update the difficulty counts below the game selector</strong></u>.
            </Alert>
            <NumberInput
            ...
            />
            <DraftChecker draftCheckState={draftCheckState} setDraftCheckState={setDraftCheckState} numPlayers={numPlayers} />
            <Text>
                ...
                <Group key={playerIndex} wrap="nowrap">
                    {Array.from(
                        difficultyCount.entries().map(([difficulty, count]) => {
                            const availableCount = playerToAvailableGoalDifficultyCounts[playerIndex].get(difficulty) ?? 0;
                            return (
                                <NumberInput
                                    key={difficulty}
                                    ...
                                    onChange={(newCount) => {
                                        if (typeof newCount === "number") {
                                            ...
                                            const newDifficultyCountsByPlayer = [...difficultyCountsByPlayer];
                                            newDifficultyCountsByPlayer[playerIndex] = newDifficultyCount;
