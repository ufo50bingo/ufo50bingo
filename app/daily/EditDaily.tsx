...
export default function EditDaily({ dailyData, date, onClose }: Props) {
    ...
    const body = password === SECRET_PASSWORD_NO_PEEKING
        ? <EditDailyBody date={date} dailyData={dailyData} onClose={onClose} />
        : (
            <Stack>
                <Text>You must provide a password to edit the Daily Bingo. If you want to contribute, please reach out to Frank on Discord.</Text>
                <TextInput placeholder="Enter password" value={password} onChange={(event) => setPassword(event.currentTarget.value)} />
                <Group justify="end">
