...
function getFeedWithoutDuplicates(
  ...
  return feed.filter((item, idx) => {
    ...
    return (
      prevItem.type !== item.type || prevItem.squareIndex !== item.squareIndex
    );
  });
}
...
export default function getDailyFeedWithoutMistakes(
  ...
  const allChanges = changesBySquare
    .flat()
    .map((pair) => pair[1])
    .concat(
...
// chronologically
function removeFeedMistakesForSquare(
  ...
  while (indexOfFirstItem >= 0) {
    const timeDiff =
      feedWithDuration[indexOfFirstItem + 1][0] -
      feedWithDuration[indexOfFirstItem][0];
    const prevIsMarked =
      indexOfFirstItem > 0
