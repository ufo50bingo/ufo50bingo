...
export default function getDailyOverlays(
  ...
  const finalMarkTimesAndDurations: (null | [number, number, number])[] = Array(25).fill(null);
  for (let i = 0; i <= feedIndex; i++) {
    ...
    if (item.type === "mark") {
      ...
      finalMarkTimesAndDurations[item.squareIndex] = [item.squareIndex, time, duration];
    } else if (item.type === "clear") {
      finalMarkTimesAndDurations[item.squareIndex] = null;
    }
  }
  const completedOnly = finalMarkTimesAndDurations.filter(tuple => tuple != null);
