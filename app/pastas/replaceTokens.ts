...
export default function replaceTokens(goal: string, tokens: Tokens): string {
  ...
  while (match != null) {
    ...
    if (values == null) {
      throw new Error(`No values defined for token ${match[0]} in goal "${goal}"`);
    }
    if (values.length === 0) {
      throw new Error(`Not enough values specified for token ${match[0]} in goal "${goal}"`);
    }
    ...
    final = final.substring(0, matchIndex) + tokenValue + final.substring(matchIndex + match[0].length);
